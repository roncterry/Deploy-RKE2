...
log:
  ...
  case ${MILVUS_TELEMETRY_ENABLED} in
    true|True)
      echo "extraConfigFiles:
  user.yaml: |+
    trace:
      exporter: jaeger
      sampleFraction: 1
      jaeger:
        url: \"http://opentelemetry-collector.${OTEL_NAMESPACE}.svc.cluster.local:14268/api/traces\"" >> ${CUSTOM_OVERRIDES_FILE}
    ;;
  esac
  echo
}
