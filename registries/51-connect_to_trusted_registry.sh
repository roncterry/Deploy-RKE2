...
CONFIG_FILE=trusted_registry_connect.cfg

TRUSTED_REGISTRY_FQDN=registry.suse.com
TRUSTED_REGISTRY_URL=oci://$(TRUSTED_REGISTRY_FQDN)/ai/charts
...
then
  ...
  if [ -z "${TRUSTED_REGISTRY_NAMESPCE_LIST}" ]
  then
    TRUSTED_REGISTRY_NAMESPACE_LIST="${*}"
    if [ -z "${TRUSTED_REGISTRY_NAMESPACE_LIST}" ]
    then
      echo
  ...
  then
    IMAGE_PULL_SECRET_NAME=suse-trusted-registry
  fi
else
  TRUSTED_REGISTRY_NAMESPACE_LIST="${*}"
  if [ -z "${TRUSTED_REGISTRY_NAMESPACE_LIST}" ]
...
log_into_trusted_registry() {
  if [ -z ${TRUSTED_REGISTRY_USERNAME} ]
  then
    # The TRUSTED_REGISTRY_URI, TRUSTED_REGISTRY_USERNAME and TRUSTED_REGISTRY_PASSWORD
  ...
  echo "COMMAND: helm registry login ${TRUSTED_REGISTRY_URL} -u ${TRUSTED_REGISTRY_USERNAME} -p ${TRUSTED_REGISTRY_PASSWORD}"
  helm registry login ${TRUSTED_REGISTRY_URL} -u ${TRUSTED_REGISTRY_USERNAME} -p ${TRUSTED_REGISTRY_PASSWORD}
  echo
}

create_trusted_registry_secret() {
  if [ -z ${TRUSTED_REGISTRY_USERNAME} ]
  then
    # The TRUSTED_REGISTRY_URI, TRUSTED_REGISTRY_USERNAME and TRUSTED_REGISTRY_PASSWORD
    # variables are set in an external file and are sourced in here:
  ...
  for APP_NAMESPACE in ${TRUSTED_REGISTRY_NAMESPACE_LIST}
  do
    if ! [ -z ${APP_NAMESPACE} ]
    then
      ...
      then
        echo "COMMAND: kubectl -n ${APP_NAMESPACE} create secret docker-registry ${IMAGE_PULL_SECRET_NAME} --docker-server=${TRUSTED_REGISTRY_FQDN} --docker-username=${TRUSTED_REGISTRY_USERNAME} --docker-password=${TRUSTED_REGISTRY_PASSWORD}"
        kubectl -n ${APP_NAMESPACE} create secret docker-registry ${IMAGE_PULL_SECRET_NAME} --docker-server=${TRUSTED_REGISTRY_FQDN} --docker-username=${TRUSTED_REGISTRY_USERNAME} --docker-password=${TRUSTED_REGISTRY_PASSWORD}
        echo
        echo "-----------------------------------------------------------------------------"
        echo
      ...
      then
        echo "COMMAND: kubectl create secret docker-registry ${IMAGE_PULL_SECRET_NAME} --docker-server=${TRUSTED_REGISTRY_FQDN} --docker-username=${TRUSTED_REGISTRY_USERNAME} --docker-password=${TRUSTED_REGISTRY_PASSWORD}"
        kubectl create secret docker-registry ${IMAGE_PULL_SECRET_NAME} --docker-server=${TRUSTED_REGISTRY_FQDN} --docker-username=${TRUSTED_REGISTRY_USERNAME} --docker-password=${TRUSTED_REGISTRY_PASSWORD}
        echo
        echo "-----------------------------------------------------------------------------"
        echo
